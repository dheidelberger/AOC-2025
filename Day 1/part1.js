...
let input = fs
    ...
    .forEach((rotation) => {
        dialValue += rotation.distance * rotation.direction;

        if (dialValue < 0) {
            const multFactor = Math.ceil(Math.abs(dialValue) / dialModulo);
            dialValue += dialModulo * multFactor;
        }
        dialValue %= dialModulo;
        if (dialValue === 0) {
            zeros++;
        }
    });
